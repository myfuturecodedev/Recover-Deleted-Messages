...
class WAGIFFragment : BaseFragment<FragmentWAGIFBinding>(FragmentWAGIFBinding::inflate) {
    ...
    private val safLauncher = registerForActivityResult(ActivityResultContracts.OpenDocumentTree()) { uri ->
        ...
        viewModel.scanGifs(requireContext())
    }
        ...
        adapter = MediaGridAdapter(
            onCardClicked = { item ->
                val bundle = Bundle().apply {
                    ...
                    putString(Constants.ARG_MEDIA_TYPE, Constants.MEDIA_TYPE_GIF)
                }
                Log.d("WAStickerFragment", "Navigating with Path: ${item.filePath}")
                findNavController().navigate(R.id.action_gifs_to_viewer, bundle)
            },
...
//        binding.rvMediaGrid.adapter = adapter
        ...
        lifecycleScope.launch {
            viewModel.gifs.collectLatest { items ->
