...
class MediaViewerFragment : BaseFragment<FragmentMediaViewerBinding>(FragmentMediaViewerBinding::inflate) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        when (mediaType) {
            ...
            Constants.MEDIA_TYPE_STICKER -> {
                binding.ivMedia.visibility = View.VISIBLE
                binding.playerView.visibility = View.GONE

                // Stickers ko scale type centerInside dena sahi rehta hai taaki stretch na ho
                binding.ivMedia.scaleType = android.widget.ImageView.ScaleType.CENTER_INSIDE

                val stickerSource: Any = if (isContentUri) Uri.parse(mediaPath) else File(mediaPath)

                Glide.with(this)
                    .load(stickerSource)
                    .placeholder(com.futurecode.recoverdeletedmessages.R.color.figma_close_btn_bg)
                    .error(com.futurecode.recoverdeletedmessages.R.color.figma_close_btn_bg)
                    .into(binding.ivMedia)
            }

            else -> { // Standard Images standard layout loading
                binding.ivMedia.visibility = View.VISIBLE
                binding.playerView.visibility = View.GONE
